...
export function computeAutoLayout(
  ...
  // For unions with only 1 child (no siblings), center the CHILD (not their spouse)
  // directly below the parent couple midpoint.
  function centerSingleChildren(): boolean {
    ...
    for (const u of unions) {
      ...
      // Center on the child's card only (not their spouse)
      const childCenterX = childPos.x + CARD_W / 2;
      const dx = parentMidX - childCenterX;

      if (Math.abs(dx) > 3) {
        childPos.x += dx;
        // Also shift spouse(s) and descendants
        for (const uid of (partnerUnions.get(cid) || [])) {
          const pu = unionMap.get(uid);
          if (!pu) continue;
          const spouseId = pu.partner1 === cid ? pu.partner2 : pu.partner1;
  ...
  // Phase 5b: Anti-crossing — detect vertical stems crossing other unions' horizontal bars
  // and shift branches apart to eliminate crossings
  function detectAndResolveCrossings(): boolean {
    let anyShift = false;
    
    // Collect all union geometries: horizontal comb bar + vertical stems
    interface UnionGeo {
      unionId: string;
      stemX: number;
      stemYTop: number;
    ...
    for (const u of unions) {
      ...
      const rightX = Math.max(p1.x, p2.x) + CARD_W;
      const unionLineY = (p1.y + p2.y) / 2 + CARD_H / 2;
      ...
      const childMinX = Math.min(...childPositions.map(p => p.x));
      const childMaxX = Math.max(...childPositions.map(p => p.x + CARD_W));
      const childMinY = Math.min(...childPositions.map(p => p.y));
      const combY = unionLineY + (childMinY - unionLineY) / 2;
      
      // Collect all members belonging to this union's branch
      const memberIdsSet = new Set<string>([u.partner1, u.partner2, ...u.children]);
      for (const cid of u.children) {
        for (const uid of (partnerUnions.get(cid) || [])) {
          const pu = unionMap.get(uid);
          if (pu) {
            memberIdsSet.add(pu.partner1);
            memberIdsSet.add(pu.partner2);
          }
        }
      }
      ...
      geos.push({
        ...
        stemYBottom: combY,
        combY,
        combLeftX: Math.min(stemX, childMinX),
        combRightX: Math.max(stemX, childMaxX),
        memberIds: memberIdsSet,
      });
    }
    
    // Check each pair of union geometries for crossings
    for (let i = 0; i < geos.length; i++) {
      for (let j = i + 1; j < geos.length; j++) {
        const a = geos[i];
        const b = geos[j];
        
        // Check if stem of A crosses comb bar of B
        const stemACrossesB = a.stemX >= b.combLeftX - 5 && a.stemX <= b.combRightX + 5 &&
                              a.stemYTop < b.combY && a.stemYBottom > b.combY;
        // Check if stem of B crosses comb bar of A
        const stemBCrossesA = b.stemX >= a.combLeftX - 5 && b.stemX <= a.combRightX + 5 &&
                              b.stemYTop < a.combY && b.stemYBottom > a.combY;
        
        if (!stemACrossesB && !stemBCrossesA) continue;
        
        // Determine which branch to shift (shift the one that's more to the right)
        const aCenterX = (a.combLeftX + a.combRightX) / 2;
        const bCenterX = (b.combLeftX + b.combRightX) / 2;
        
        const [leftGeo, rightGeo] = aCenterX <= bCenterX ? [a, b] : [b, a];
        const overlap = leftGeo.combRightX - rightGeo.combLeftX + BRANCH_GAP;
        
        if (overlap > 0) {
          // Shift right branch's members to the right
          for (const mid of rightGeo.memberIds) {
            const pos = positions.get(mid);
            if (pos) pos.x += overlap;
          }
          // Update geometry
          rightGeo.combLeftX += overlap;
          rightGeo.combRightX += overlap;
          rightGeo.stemX += overlap;
          anyShift = true;
        }
      }
    }
  ...
  // Run crossing resolution
  for (let pass = 0; pass < 5; pass++) {
    if (!detectAndResolveCrossings()) break;
    // Re-resolve collisions after shifting

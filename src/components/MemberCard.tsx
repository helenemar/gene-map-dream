...
const EDGE_DOTS: { side: AnchorSide; pos: React.CSSProperties }[] = [
  { side: 'top',    pos: { top: 0, left: '50%' } },
  { side: 'right',  pos: { top: '50%', right: 0 } },
  { side: 'bottom', pos: { bottom: 0, left: '50%' } },
  { side: 'left',   pos: { top: '50%', left: 0 } },
];
      ...
      {activeState === 'anchor-active' && !presentationMode && (
        ...
        >
          <button
            ...
            className="flex items-center gap-1.5 px-4 h-9 rounded-full bg-primary text-primary-foreground text-xs font-semibold shadow-md hover:bg-primary/90 hover:shadow-lg active:scale-[0.97] transition-all whitespace-nowrap cursor-crosshair"
  ...
  return (
    <motion.div
      ...
      animate={{
        left: member.x,
        top: member.y,
        opacity: isFadingOut ? 0 : searchDimmed ? 0.6 : 1,

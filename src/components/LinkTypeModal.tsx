...
const LinkTypeModal: React.FC<LinkTypeModalProps> = ({ open, onSelect, onClose, currentType, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl shadow-modal p-5 w-[340px] max-h-[75vh] flex flex-col overflow-hidden"
        ...
        >
          {EMOTIONAL_LINK_TYPES.map((lt) => (
            <button
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-foreground hover:bg-accent transition-colors text-left shrink-0 border border-transparent ${
                currentType === lt.id ? 'bg-primary/10 border-primary/30' : ''
              }`}
            >
              <div

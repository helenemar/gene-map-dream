...
CREATE POLICY "Users can view their own genograms"
  ON public.genograms FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own genograms"
  ON public.genograms FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own genograms"
  ON public.genograms FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own genograms"
...
-- Indexes
CREATE INDEX idx_genograms_user_id ON public.genograms (user_id);
